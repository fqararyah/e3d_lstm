...
def gradients(ys, xs, grad_ys=None, checkpoints='collection', **kwargs):
    ...
    if type(checkpoints) is not list:
        if checkpoints == 'collection':
            ...
            # Fareed just printing
            checkpoints = ge.filter_ts_from_regex(fwd_ops, 'mul')
            print('++++++++++++++++++++++++++++++++++++++')
            ...
            for ts in [ts_filtered, ts_all]:
                ...
                for t in ts:
                    ...
                    if not set(b_inp).intersection(f_inp) and len(b_inp)+len(f_inp) >= 50:
    ...
    print(len(checkpoints_sorted_lists[::-1]))
    iii = 0
